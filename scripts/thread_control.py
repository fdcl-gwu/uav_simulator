...
class ControlNode(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(WrenchStamped, '/uav/fm', 10)
    ...
    def timer_callback(self):
        ...
        if (not rover.motor_on) or (rover.mode < 2):
            fM_message = WrenchStamped(force=Vector3(x=0.0, y=0.0, z=0.0), \
                torque=Vector3(x=0.0, y=0.0, z=0.0))
        else:
            fM_message = WrenchStamped(force=Vector3(x=0.0, y=0.0, z=fM[0][0]), \
